...
const isNoWpBlock = process.argv.includes('--nowp');
...
async function createFlsComponent() {
	...
	if (!name || /[а-яА-ЯёЁіїєґІЇЄҐ0-9\s\p{P}]/gu.test(name)) {
		...
		const folderPathInWp = findFolderRecursive(
			`src/wordpress/fls-theme/components/blocks`,
			name,
		);
		if (
			fs.existsSync(`src/components/custom/${name}`) ||
			(folderPathInHTML && folderPathInWp)
		) {
...
<section data-fls-${name} class="${name}">
	<div class="${name}__container">
			...
			// Create WP Block
			if (!folderPathInWp && !isNoWpBlock) {
				fs.mkdirSync(`${phpPath}/${name}`);
				fs.writeFileSync(
					`${phpPath}/${name}/${name}.php`,
					`
