...
export function navPanel() {
	...
	if (
		...
		if (!isProduction) {
			templateConfig.projectpage.enable
				? (menu += `<li><hr></li><li><a target="_blank" href="${templateConfig.projectpage.template.replace('src', '')}">Шаблон проекта</a></li>`)
				: '';
			isIconFont
			...
			templateConfig.devcomponents.enable
				? (menu += `<li><hr></li><li><a target="_blank" href="${templateConfig.devcomponents.filename}">Компоненты</a></li>`)
				: '';
		}
		menu += `</ul>`;
		menu += `<style>
